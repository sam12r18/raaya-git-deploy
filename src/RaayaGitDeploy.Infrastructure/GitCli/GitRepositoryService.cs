...
public sealed class GitRepositoryService : IGitRepositoryService
{
    ...
    public GitRepositoryService(IGitProcessRunner runner) => _runner = runner ?? throw new ArgumentNullException(nameof(runner));

    public async Task<GitRepositoryContext> GetContextAsync(string path, CancellationToken cancellationToken)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(path);
        var root = await RunRequiredAsync(path, ["rev-parse", "--show-toplevel"], cancellationToken);
        var headSha = await RunRequiredAsync(root, ["rev-parse", "HEAD"], cancellationToken);
        var branchResult = await _runner.RunAsync(root, ["branch", "--show-current"], cancellationToken);
        if (branchResult.ExitCode != 0) throw new InvalidOperationException(BuildGitFailureMessage(["branch", "--show-current"], branchResult));
        var branchName = branchResult.StandardOutput.Trim();
        if (string.IsNullOrWhiteSpace(branchName)) branchName = $"detached@{headSha[..Math.Min(8, headSha.Length)]}";
    ...
    public async Task<IReadOnlyList<GitCommitInfo>> GetRecentCommitsAsync(string repositoryPath, int limit, CancellationToken cancellationToken)
    {
    ...
    public async Task<string> GetCommitFileDiffAsync(string repositoryPath, string commitSha, string repositoryRelativePath, CancellationToken cancellationToken)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(repositoryPath);
        ArgumentException.ThrowIfNullOrWhiteSpace(repositoryRelativePath);
        var root = await RunRequiredAsync(repositoryPath, ["rev-parse", "--show-toplevel"], cancellationToken);
        var gitPath = NormalizeGitPath(repositoryRelativePath);
        ValidateRepositoryRelativePath(root, gitPath);
        var commit = await ResolveCommitAsync(root, commitSha, cancellationToken);
        var arguments = new[] { "show", "--format=", "--no-color", commit, "--", gitPath };
        var result = await _runner.RunAsync(root, arguments, cancellationToken);
        if (result.ExitCode != 0) throw new InvalidOperationException(BuildGitFailureMessage(arguments, result));
    ...
    public Task<string> GetDiffAsync(string repositoryPath, string path, string? baseRef, CancellationToken cancellationToken) => GetDiffAsync(repositoryPath, path, baseRef, DefaultMaxUntrackedPreviewBytes, cancellationToken);

    public async Task<string> GetDiffAsync(string repositoryPath, string path, string? baseRef, long maxUntrackedPreviewBytes, CancellationToken cancellationToken)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(repositoryPath);
        ArgumentException.ThrowIfNullOrWhiteSpace(path);
        ...
        ValidateRepositoryRelativePath(root, gitPath);
        string[] arguments;
        if (baseRef is null) arguments = ["diff", "--no-color", "HEAD", "--", gitPath];
        else { var baseCommit = await ResolveCommitAsync(root, baseRef, cancellationToken); arguments = ["diff", "--no-color", $"{baseCommit}...HEAD", "--", gitPath]; }
        var result = await _runner.RunAsync(root, arguments, cancellationToken);
        if (result.ExitCode != 0) throw new InvalidOperationException(BuildGitFailureMessage(arguments, result));
        ...
        string text;
        try { text = StrictUtf8.GetString(bytes); } catch (DecoderFallbackException) { return string.Empty; }
    ...
    private static IReadOnlyList<GitCommitInfo> ParseCommitHistory(string output)
    {
        ...
        foreach (var record in output.Split(CommitRecordSeparator, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
        {
            ...
            if (!DateTimeOffset.TryParse(fields[3].Trim(), CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out var authorDate)) throw new InvalidOperationException("git log returned an invalid author date.");
            var sha = fields[0];
            commits.Add(new GitCommitInfo(sha, sha[..Math.Min(8, sha.Length)], fields[1], fields[2], authorDate));
        }
        return commits;
    }

    private async Task<string> ResolveCommitAsync(string repositoryRoot, string commitRef, CancellationToken cancellationToken)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(commitRef);
        return await RunRequiredAsync(repositoryRoot, ["rev-parse", "--verify", "--end-of-options", $"{commitRef}^{{commit}}"], cancellationToken);
    }
    ...
    {
        if (Path.IsPathRooted(path)) throw new ArgumentException("Diff path must be relative to the repository root.", nameof(path));
        var root = Path.GetFullPath(repositoryRoot);
        var candidate = Path.GetFullPath(Path.Combine(root, path.Replace('/', Path.DirectorySeparatorChar)));
        var relative = Path.GetRelativePath(root, candidate);
        if (relative == ".." || relative.StartsWith($"..{Path.DirectorySeparatorChar}", StringComparison.Ordinal)) throw new ArgumentException("Diff path must remain inside the repository root.", nameof(path));
    }
    ...
    private static string BuildAllAddedPreview(string path, string text)
    {
        ...
        preview.AppendLine("--- /dev/null"); preview.AppendLine($"+++ b/{path}"); preview.AppendLine($"@@ -0,0 +1,{lineCount} @@");
