...
public sealed class GitRepositoryServiceTests
{
    ...
    [Fact]
    public async Task GetDiffAsync_UntrackedTextFile_ReturnsAllAddedPreview()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var root = Path.Combine(Path.GetTempPath(), $"rgd-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            const string relativePath = "notes new.txt";
            await File.WriteAllTextAsync(
            ...
            Assert.Contains("+++ b/notes new.txt", diff, StringComparison.Ordinal);
            Assert.Contains("+first line", diff, StringComparison.Ordinal);
            Assert.Contains("+second line", diff, StringComparison.Ordinal);
        }
        finally
        {
            DeleteDirectory(root);
    ...
    [Fact]
    public async Task GetDiffAsync_UntrackedBinaryFile_DoesNotRenderTextPreview()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var root = Path.Combine(Path.GetTempPath(), $"rgd-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            const string relativePath = "asset.bin";
            await File.WriteAllBytesAsync(
                Path.Combine(root, relativePath),
                [0x52, 0x47, 0x44, 0x00, 0x01, 0x02, 0xFF],
                cancellationToken);
